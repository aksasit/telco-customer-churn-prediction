import os 
import sys
import pandas as pd

# Make sure python can find src package
sys.path.append(os.path.abspath("src"))

from data.load_data import load_data
from data.preprocess import preprocess_data
from features.build_features import build_features

# === CONFIG ===
DATA_PATH = "/Users/DELL/Downloads/Telco_Customer_Churn_Prediction/data/raw/telco_customer_churn.csv" 
TARGET_COL = "Churn"

def main():
    print("=== Testing Phase 2: Load → Preprocess → Build Features ===")
    
    # 1 - Load Data
    print("\n[1] Loading data...")
    df = load_data(DATA_PATH)
    print(f"Data Loaded. Shape: {df.shape}")
    
    # 2 - Preprocess
    print("\n[2] Preprocessing data...")
    df_clean = preprocess_data(df, target_col=TARGET_COL)
    print(f"Data after prerocessing. Shape: {df_clean.shape}")
    print(df_clean.head(2))
    
    # 3 - Build Features
    print("\n[3] Building features...")
    df_features = build_features(df_clean, target_col=TARGET_COL)
    print(f"Data after feature engineering. Shape: {df_features.shape}")
    print(df_features.head(2))
    
    print("\n✅ Phase 1 pipeline completed successfully!")
    
if __name__ == "__main__":
    main()    